...
window.Mousetrap = function() {
	function o(a, c, b) {
		if (a.addEventListener) return a.addEventListener(c, b, !1);
	...
	function w(a, c, b, d, C) {
		...
		if (!j[a]) return [];
		"keyup" == b && q(a) && (c = [a]);
		for (g = 0; g < j[a].length; ++g) if (e = j[a][g], !(e.seq && l[e.seq] != e.level) && b == e.action && ("keypress" === b || c.sort().join(",") === e.modifiers.sort().join(","))) d && e.combo == C && j[a].splice(g, 1), f.push(e);
		return f
	}
	function r(a, c) {
		!1 === a(c) && (c.preventDefault && c.preventDefault(), c.stopPropagation && c.stopPropagation(), c.returnValue = !1, c.cancelBubble = !0)
	}
	function s(a) {
		a.which = "number" == typeof a.which ? a.which : a.keyCode;
		var c = u(a);
		if (c) if ("keyup" === a.type && t === c) t = !1;
		else {
			...
			if (!(-1 < (" " + b.className + " ").indexOf(" mousetrap ") ? 0 : "INPUT" == d || "SELECT" == d || "TEXTAREA" == d)) {
				...
				for (var b = w(c, b, a.type), f = {}, g = !1, d = 0; d < b.length; ++d) b[d].seq ? (g = !0, f[b[d].seq] = 1, r(b[d].callback, a)) : !g && !n && r(b[d].callback, a);
				a.type == n && !q(c) && p(f)
			}
		}
	}
	...
	function x(a, c, b) {
		if (!b) {
			if (!k) {
				k = {};
				for (var d in h) 95 < d && 112 > d || h.hasOwnProperty(d) && (k[h[d]] = d)
			}
	...
	function y(a, c, b, d, f) {
		...
		if (1 < g.length) {
			...
			for (d = 0; d < g.length; ++d) y(g[d], d < g.length - 1 ? a : b, m, k, d)
		} else {
			h = "+" === a ? ["+"] : a.split("+");
			for (g = 0; g < h.length; ++g) e = h[g], A[e] && (e = A[e]), b && ("keypress" != b && B[e]) && (e = B[e], i.push("shift")), q(e) && i.push(e);
			b = x(e, i, b);
			j[e] || (j[e] = []);
			w(e, i, b, !d, a);
			j[e][d ? "unshift" : "push"]({
	...
	for (var h = {
		...
		"return": "enter",
		escape: "esc"
	}, k, j = {}, i = {}, l = {}, z, t = !1, n = !1, f = 1; 20 > f; ++f) h[111 + f] = "f" + f;
	for (f = 96; 106 > f; ++f) h[f] = f - 96;
	o(document, "keypress", s);
	o(document, "keydown", s);
	o(document, "keyup", s);
	return {
		bind: function(a, c, b) {
			for (var d = a instanceof Array ? a : [a], f = 0; f < d.length; ++f) y(d[f], c, b);
			i[a + ":" + b] = c
		},
		unbind: function(a, c) {
...
$(function() {
	$.getJSON('http://sou.qq.com/online/get_weather.php?callback=?', function(data) {
		$('#weather').html('<img style="float:right;margin-top:6px;" src="http://cache.soso.com/zdq/tb/day/b' + data.future.bwea_0_icon.substr(1) + '" />' + '<cite>' + data.future.name + '</cite>　实时气温' + data.real.temperature + '℃' + '<br />今天 ' + data.future.tmin_0 + '℃～' + data.future.tmax_0 + '℃ ' + data.future.wea_0 + '<br />明天 ' + data.future.tmin_1 + '℃～' + data.future.tmax_1 + '℃ ' + data.future.wea_1);
	});
...
function share(engine) {
	var urls = {
		'sina': 'http://service.weibo.com/share/share.php?',
		'sohu': 'http://t.sohu.com/third/post.jsp?',
		'qq': 'http://v.t.qq.com/share/share.php?'
	};
	var opt = ['url=', encodeURIComponent(location.href), '&title=', encodeURIComponent(document.title)];
	if (engine == 'sohu') {
		opt.push('&content=', 'utf-8');
	}
	url = urls[engine] + opt.join('');
